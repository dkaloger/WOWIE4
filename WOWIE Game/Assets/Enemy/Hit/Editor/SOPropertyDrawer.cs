        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (property.isArray)
            {
                for (int i = 0; i < property.arraySize; ++i)
                    ShowDropdownForProperty(position, property.GetArrayElementAtIndex(i), label, list, names);
            }
            else
            {
                ShowDropdownForProperty(position, property, label, list, names);
            }
        }

        private void ShowDropdownForProperty(Rect position, SerializedProperty property, GUIContent label, List<ScriptableObject> list, IEnumerable<string> names)
        {
