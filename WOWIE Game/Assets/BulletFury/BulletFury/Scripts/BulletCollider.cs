...
namespace BulletFury
{
    ...
    public class BulletCollider : MonoBehaviour
    {
        ...
        #if UNITY_EDITOR
        [UnityEditor.MenuItem("BulletFury/Validate")]
        public static void Validate()
        {
            var colliders = FindObjectsOfType<Collider2D>();
            foreach (var c in colliders)
                if (c.GetComponent<BulletCollider>() == null)
                    Debug.LogWarning($"Object {c.name} doesn't have a bullet collider attached, it won't check bullet collisions!");

            var bulletColliders = FindObjectsOfType<BulletCollider>();
            foreach (var c in bulletColliders)
                if (c.bulletLayersToBeHitBy == 0)
                    Debug.LogWarning($"Bullet collider {c.name} doesn't have any bullet layers set, it won't collide with any bullets!");
        }
        #endif
    }
}