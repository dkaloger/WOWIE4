...
public class Shearing : MonoBehaviour
{
     ...
     private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.name.Contains("Ore"))
        {
            ...
            collision.transform.parent.parent.GetComponent<PlayerController>().Helditem = Initialize(Wool, new Vector2(collision.transform.parent.parent.GetComponent<Transform>().position.x, collision.transform.parent.parent.GetComponent<Transform>().transform.y),Quaternion.identity);
