...
public class Shearing : MonoBehaviour
{
     ...
     private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.name.Contains("Ore"))
        {
            ...
            collision.transform.parent.parent.GetComponent<PlayerController>().Helditem = Initialize(wool, new Vector2(collision.transform.parent.parent.GetComponent<Transform>().position.x, collision.transform.parent.parent.GetComponent<Transform>().position.y),Quaternion.identity);
