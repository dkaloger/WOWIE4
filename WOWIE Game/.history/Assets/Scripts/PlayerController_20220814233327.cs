   ...
   public bool Line17;
    ...
    void Update()
    {
        ...
        if(Helditem != null && Helditem.name.Contains("Wool")){
            ...
            if(GetComponent<SpriteRenderer>().sprite.name.Contains("backward")){
                Helditem.GetComponent<Transform>().position = new Vector2(transform.position.x,transform.position.y-0.3f);
                Helditem.GetComponent<SpriteRenderer>().sortingOrder = 0;
            }else{
