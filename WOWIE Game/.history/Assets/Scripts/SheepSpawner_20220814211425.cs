    ...
    private void Spawn(){
        ...
        if(zonesCount[i]<10 && i > -1){
            ...
            while(screenPosition.y > Screen.height || screenPosition.y <0 || screenPosition.x > Screen.width || screenPosition.x < 0){
                Destroy(sh);
                GameObject sh2 = Instantiate(sheep, new Vector2(Random.Range(zoneBounds[i][0],zoneBounds[i][1]),Random.Range(zoneBounds[i][2],zoneBounds[i][3])), Quaternion.identity);
                Vector2 screenPosition = Camera.main.WorldToScreenPoint(sh2.transform.position);
            }
            zonesCount[i]++;
        }
    }
