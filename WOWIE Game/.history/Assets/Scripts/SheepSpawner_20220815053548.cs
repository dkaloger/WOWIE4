    ...
    void Update()
    {
        if(!(screenPosition.y > Screen.height || screenPosition.y <0 || screenPosition.x > Screen.width || screenPosition.x < 0)){
            ...
            if(seen){
                ...
                GameObject sh2 = Instantiate(sheep, new Vector2(sh2.position.x + Random.Range(-5,5), sh2.position.y + Random.Range(-5,5)), Quaternion.identity);
                GameObject sh3 = Instantiate(sheep, new Vector2(s.x + Random.Range(-5,5), sheepPos.y + Random.Range(-5,5)), Quaternion.identity);
