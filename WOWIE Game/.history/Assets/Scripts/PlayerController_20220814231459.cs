   ...
   public bool Line17;
    ...
    void Update()
    {
        ...
        if(Helditem != null && Helditem.name.Contains("Wool")){
            if(GetComponent<SpriteRenderer>().sprite.name.Contains == "playerforward_0") Helditem.GetComponent<Transform>().position = transform.position;
            if(GetComponent<SpriteRenderer>().sprite.name == "playerforward_0") Helditem.GetComponent<Transform>().position = new Vector2(transform.position.x+1,transform.position.y);
        }
